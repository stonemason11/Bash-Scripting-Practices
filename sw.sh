...
# %s: second since 1940-01-01 00:00:00 UTC 
# %N: Nano seconds
# grep -qF 'quiet' and 'fixed strings'
date +%s.%N | grep -qF 'N' && use_python=1 # of `date` lacks nanoseconds
...
fmt_seconds(){
  ...
  if [ "$mins" != "0" ]; then
    seconds=`echo "$seconds - ($min*60)" | bc`
    echo "$mins:$seconds"
  else
    echo $seconds
  fi
...
while true; do
  ...
  if [ $ret -eq 1 -o $ret -eq 2 -o $ret -eq 130 ] ; then 
    [ ! "$start" ] && { echo >&2; exit; }
    stop
