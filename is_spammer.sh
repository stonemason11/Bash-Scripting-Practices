...
# $'...': expand an escaped hex number to ASCII character
...
# # # Debug version only - remove for regulare use # # #
#
...
pend_pop() {
	if [ ${#_pending_[@]} -gt 0 ]; then
		local -i _top_
		_top_=${#_pending_[@]}-1	# array uses 0 based index
		unset _pending_[$_top_]		# unset array[3+2] works
						# must not use ${}
						# echo $array[3+2] doesn't
						# echo ${array[3+2]} does
	fi
}

# pend_func function name [$(printf '%g\n' arguments)]
pend_func(){
	local IFS=${NO_WSP}
	# disable file name generation (globing)
	set -f			
	_pending_[${#_pending_[@]}]=$@
	set +f
}

# The function stops the release:
pend_stop_mark(){
	_p_ctrl_=0
}
...
pend_release() {
	...
	_p_ctrl_=${#_pending[@]}
	while [ ${_p_ctrl_} -gt 0 ]; do
		_top_=${#_pending_[@]}-1
		_pend_current=${_pending_[$_top_]}
...
pend_drop () {
	local -i _top_
	local _pd_ctrl=${#_pending[@]}
	while [ ${_p_ctrl_} -gt 0 ]	
	# _p_ctrl is changed by evaluating function pend_stop_mark
	do
		_top_=$_pd_ctrl_-1
...
function edit_by_glob(){
	...
	do
		...
		do
			...
			then
				_ebg_t=${_ebg_Target[$n]/#${_ebg_x}/}	# string delete match from beginning
...
to_lower() {
	[ $# -eq 1 ] || return 1
	local _t1_out
	_tl_out=${1//A/a}		# ${var//pattern/replacement} repalces all
	_tl_out=${_tl_out//B/b}
	_tl_out=${_tl_out//C/c}
	_tl_out=${_tl_out//D/d}
	_tl_out=${_tl_out//E/e}
...
# Note everybody uses dots as separators (ASPNIC, for example)
# This function described in to_do.bash
# to_do <string>
to_dot(){
	[ $# -eq 1 ] || return 1
	echo ${1//[#|@|%]/.}
...
# file_to_array <filename> < line_array_name>
file_to_array(){
	[ $# -eq 2 ] || return 1	# two argument required
	local IFS=${NO_WSP}
	local -a _fta_tmp_
	_fta_tmp=( $(cat $1) )
	eval $2=\( \$\{_fta_tmp_\[@\]\} \)
...
# Original input IP address expansion run
...
been_there_addr=( '127.0.0.1' )

# Name which have checked (or given up on)
...
# Which will also set _dot_dum to this function . . .
dump_dot(){
	local -i _dd_cnt
	echo '# Data vintage: '$(data -R) > ${_dot_file}
...
# _<service>._<protocol>.<domain_name> 3600 IN SRV 0 0 389 ldap.openldap.org
...
long_rev() {
	...
	echo -n ':'
	# echo 'lrev: '${1}
	_lr_reply=( $( dig +noall +nofail +answer +authority +additional \
