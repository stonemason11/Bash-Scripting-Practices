...
ROWS=5 # Five rows of pegs (matchsticks)
WON=91 # Exit codes to keep track of wins/losses
LOST=92 # Possibly useful if running in match mode
QUIT=99
peg_msg= #Pge/Pegs?

Rows=( 0 5 4 3 2 1 ) # Array hold in play info
# ${Rows[0]} holds the total number of pegs, updated after each turn
# Other array elements hold number of pegs in corresponding row.

instructions(){
  clear
  tput bold # turn on bold mode, see man terminfo for full list 
...
INSTRUCTIONS
    ...
    echo -e "\033[0m" # ESC=\e=\033, m : set graphics mode, 0: all attributes off

  # Restore display. sgr0: turn off all attributes
...
tally_up(){
  let "Rows[0] = ${Rows[1] }+ ${Rows[2]} + ${Rows[3]} + ${Rows[4]} + ${Rows[5]}"
}
