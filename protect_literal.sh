...
# Pick an un0used, non-printing character as local IFS.
# Not required, but shows that we are ignoring it.

	local IFS=$'\x1B' # \ESC character # \x1B is \ESC in ASCII

# Enclose the All-Elements-Of in hard quatoes during assignment.

	local tmp=$'\x27'$@$'\x27'  # ASCII \x27 is '
...
# The difference is hidden inside the Bash interal order
#+ of operations.
...
# Now list thant array and see what is there
echo "- - Test Two - -"
for (( i=0 ; i<${#arrayZ[@]} ; i++ ))
do
